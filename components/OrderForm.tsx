...
type OrderFormProps = {
  ...
  chatClosed: boolean
  ticketId?: string
  handleSubmit: (data: { username: string; address: string; deliveryNotes: string; paymentMethod: string }) => void
            ...
            {validationResult.error ? (
              <div className="flex items-center space-x-4 text-orange-300">
                ...
                <span className="font-bold text-xl text-white">
                  Hmm, something seems off with your group link. It might be invalid or from a store we don’t support
                  yet. Please review the link or contact our support team — we’re happy to help!
                </span>
              </div>
            ) : (
                      ...
                      <>
                        <div className="bg-black/80 rounded-2xl p-8 space-y-6 border border-orange-500/40 glow-orange">
                          {validationResult.items.map((item, index) => (
                            <div
                              key={index}
                              className="flex justify-between items-center border-b border-orange-500/20 pb-4"
                            >
                              <div>
                        ...
                        <div className="bg-black/80 rounded-2xl p-8 space-y-6 border border-orange-500/40 glow-orange">
                          <h3 className="text-white font-black text-2xl mb-4">Delivery Information</h3>
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className="w-full bg-neutral-900 border-2 border-orange-500/30 rounded-xl px-4 py-3 text-white text-base font-medium placeholder-gray-500 transition-all duration-300 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/50"
                          ...
                          <div>
                            <label className="block text-lg font-bold text-white mb-3">Delivery Notes</label>
                            <textarea
                              placeholder="Any special instructions? (e.g., gate code, apartment number)"
                              className="w-full bg-neutral-900 border-2 border-orange-500/30 rounded-xl px-4 py-3 text-white text-base font-medium placeholder-gray-500 transition-all duration-300 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/50 min-h-[100px] resize-y"
                              value={deliveryNotes}
